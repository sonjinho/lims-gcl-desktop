...
export default function IEC62552_ExportData(
  ...
  for (let i = 0; i < rawData.length; i++) {
    dateTimeData.push(new Date(rawData[i][ExcelColumn.DateTime]));
    powerData.push(rawData[i][ExcelColumn.Power]);
  }
  ...
  for (let i = 0; i < cycleData.length - numberOfTCC * 3 - 1; i++) {
    ...
    row.testPeriodUnfrozenFresh = getTestPeriodAverage(
      rawData,
      startIndex,
      endIndex,
      [ExcelColumn.TC_3, ExcelColumn.TC_4, ExcelColumn.TC_5]
    );
    row.testPeriodUnfrozenCellar = getTestPeriodAverage(
      rawData,
      startIndex,
      endIndex,
      [ExcelColumn.TC_6]
    );
    row.testPeriodFrozen3Star = getTestPeriodAverage(
      rawData,
      startIndex,
      endIndex,
      [
    ...
    row.ambientTemp = getTestPeriodAverage(rawData, startIndex, endIndex, [
      ExcelColumn.TC_1,
      ExcelColumn.TC_2,
    ]);
    ...
    row.blockAStart = formatLocalTime(dateTimeData[cycleData[blockA[0]].index])
    row.blockBStart = formatLocalTime(dateTimeData[cycleData[blockB[0]].index])
    row.blockCStart = formatLocalTime(dateTimeData[cycleData[blockC[0]].index])
...
const COLUMN_MAPPING: Record<keyof ExportRow, string> = {
  ...
  blockB: "Block B",
  blockC: "Block C",
  testPeriodUnfrozenFresh: "Unfrozen Fresh (°C)",
  testPeriodUnfrozenCellar: "Unfrozen Cellar (°C)",
  testPeriodFrozen3Star: "Frozen 3-Star (°C)",
  testPeriodFrozen2Star: "Frozen 2-Star (°C)",
  testPeriodPower: "Power (kWh)",
  testPeriodABC: "Test Period (h)",
  ambientTemp: "Ambient Temperature (°C)",
  spreadUnfrozen: "Spread Unfrozen (°C)",
  spreadFrozen: "Spread Frozen (°C)",
  spreadPower: "Spread Power (kWh)",
  slopeUnfrozen: "Slope Unfrozen (°C/h)",
  slopeFrozen: "Slope Frozen (°C/h)",
  slopePower: "Slope Power (°C/h)",
  permittedPowerSpread: "Permitted Power Spread (kWh)",
  valid: "Valid",
  testPeriodValid: "Test Period Valid",
  blockAStart: "Block A Start Time",
  blockBStart: "Block B Start Time",
  blockCStart: "Block C Start Time"
};
  ...
  return data.map((row) => {
    ...
    for (const key in row) {
      if (COLUMN_MAPPING[key as keyof ExportRow]) {
        transformedRow[COLUMN_MAPPING[key as keyof ExportRow]] = row[key as keyof ExportRow];
      }
    }
...
class ExportRow {
  ...
  testPeriodUnfrozenFresh: number;
  testPeriodUnfrozenCellar: number;

  testPeriodFrozen3Star: number;
  testPeriodFrozen2Star: number;
